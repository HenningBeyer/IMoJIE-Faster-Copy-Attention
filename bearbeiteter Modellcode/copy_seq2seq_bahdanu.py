...
class CopyNetSeq2Seq(Model):
    ...
    def __init__(self,
                 ...
                 do_not_combine_copy_log_probs: bool = False,
                 do_not_copy_out_of_extractions: bool = False,
                 ) -> None:

        super().__init__(vocab)
        
        self._do_not_combine_copy_log_probs = do_not_combine_copy_log_probs
        self._do_not_copy_out_of_extractions = do_not_copy_out_of_extractions
        ...
        #print(self.vocab.get_token_to_index_vocabulary('bert')) --> {'bert': {'@@PADDING@@': 0, '@@UNKNOWN@@': 1}}
        #print(self.vocab.get_token_to_index_vocabulary('roebrta'))
        #print(self.vocab.get_token_to_index_vocabulary(source_namespace)) 
        #print(self.vocab.get_token_to_index_vocabulary(target_namespace))
        #print('testABC')
        #print('GETVOCAB SIIIIZE', self.vocab.get_vocab_size(self._target_namespace))
        #print(self.vocab.get_token_index('man', self._target_namespace ))
               
        if self._bert:
            START_SYMBOL,END_SYMBOL = bert_utils.init_globals(not self._roberta)
            self._target_vocab_size = 28996 
            self.token_mapping = bert_utils.mapping
        elif self._roberta:
            START_SYMBOL,END_SYMBOL = bert_utils.init_globals(not self._roberta)
            self._target_vocab_size = 50265 + 100 
            self.token_mapping = bert_utils.mapping
            
        elif self._electra:
            START_SYMBOL,END_SYMBOL = bert_utils.init_globals(not self._roberta)
            self._target_vocab_size = 30522
            self.token_mapping = bert_utils.mapping   
        elif self._deberta:
            START_SYMBOL,END_SYMBOL = bert_utils.init_globals(not self._roberta)
            self._target_vocab_size = 50265 + 100
            self.token_mapping = bert_utils.mapping
        elif self._debertav3: # has tokens '<0x00>': 4, '<0x01>': 5, '<0x02>': 6, ... but using regular unused tokens
            START_SYMBOL,END_SYMBOL = bert_utils.init_globals(not self._roberta)
            self._target_vocab_size = 128100 + 100
            self.token_mapping = bert_utils.mapping
            ...
            self.masked_padding_token_id = 0
        # Encoding modules.
    ...
    def initialize(self):
        ...
        if self._bert or self._electra or self._deberta or self._debertav3:
            ...
            self.vocab._oov_token = '<unk>' #bert has [UNK] but uses [unused99] as oov token as above
            self.vocab._padding_token = '<pad>' #has index 1 and bert has [PAD] with index 0
        
        if self._roberta or self._deberta or self._debertav3:
            bert_unused_tokens = [f"[unused{i}]" for i in range(1,100+1)]
            self.vocab.add_tokens_to_namespace(bert_unused_tokens, self._source_namespace) 
        ...
        self.vocab.get_token_index('[unused1]', self._target_namespace, True)
        ...
        print('Vocab-Check: ', self.start_arg1, self.end_arg1, self.start_arg2, self.end_arg2, self.start_rel, self.end_rel)
        print(self._copy_index, self._eoe_index, self._pad_index, self._oov_index)
        print(self._end_index, self._start_index, self._src_end_index, self._src_start_index)
    ...
    def forward_append(self, source_tokens, source_token_ids, source_to_target, metadata, target_tokens, target_token_ids, optimizer):
        print(metadata, target_tokens, target_token_ids)
        if target_tokens:
    ...
    def test_append(self, source_tokens, source_token_ids, source_to_target):
        ...
        #print('test_source_tokens rigth?:', source_tokens['tokens'])
        ...
        while not stop:
            ...
            for bs in range(batch_size):
                ...
                if not (self._bert or self._roberta or self._electra or self._deberta or self._debertav3):
    ...
    def train_append(self, source_tokens, source_token_ids, source_to_target, metadata, target_tokens, target_token_ids, optimizer):
        # (batch_size, source_tokens_length)
        #print('meta_data:', metadata, 'training target tokens:', target_tokens)
        append_tokens = source_tokens['tokens']
        unp_append_tokens = self._remove_padding(append_tokens)
        ...
        for extraction_num in range(num_extractions):
            ...
            if self._teacher_forcing:
                ...
                with torch.no_grad():
                    ...
                    unp_prediction_tokens = self._remove_padding(prediction, index=self._end_index)
                    print('Beam Search Predictions:', unp_prediction_tokens) 
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        #print(f'Shape Source Token: {source_tokens["tokens"].shape}')
        #print(f'Length Source Token: {source_tokens["tokens"][1]}')
        #print(f'Source Token: {source_tokens["tokens"][8]}')
        t1 = time.time()
        ...
        try:
            ...
            #print('Tokens to encode', source_tokens['tokens'], embedded_input)
            embedded_input_cpu = embedded_input.cpu()
        except:
            ipdb.set_trace()

        # shape: (batch_size, max_input_sequence_length)
        source_mask = util.get_text_field_mask(source_tokens, masked_token_id=self._pad_index)
        # shape: (batch_size, max_input_sequence_length, encoder_output_dim)
        encoder_outputs = self._encoder(embedded_input, source_mask)
        #print('sourcemask', source_mask)
        #print(source_tokens['tokens'])
        
        #print(f'Encoding time: {time.time() -t1} s')
    ...
    def _decoder_step(self,
        ...
        #print('Source mask: ', state["source_mask"])
    ...
    def _forward_loss(self,
        ...
        #print('source tokens right for model --> tokenization??; source and target_tokens:', state["source_token_ids"], target_token_ids)
        ...
        for timestep in range(num_decoding_steps):
            ...
            if timestep < num_decoding_steps - 1:
                ...
                #print('copyied tokens: ', (target_to_source.sum(-1) > 0).long())
                # shape: (batch_size,)
        ...
        # the END token.
        # shape: (batch_size, target_sequence_length)
        target_mask = util.get_text_field_mask(target_tokens, masked_token_id=self._pad_index)
    ...
    def _forward_beam_search(self, state: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        #print(f'Decoding time: {time.time() -t1} s')
        return output_dict
    ...
    def _gather_final_log_probs(self,
        ...
        """ Original implementation of _gather_final_log_probs""" 

        if (not self._do_not_combine_copy_log_probs and not self._do_not_copy_out_of_extractions):  
            #print('original gather_final_log_probs')   
        ...
        ### Implementation with still copying out of all extractions but without combining copy log probs ###

            #removed if statements in for loop; they were for ignoring allready summed copy log probs and adding future copy log probs
    
        elif self._do_not_combine_copy_log_probs and not self._do_not_copy_out_of_extractions: 
            #print('no combine gather_final_log_probs')  
                
            _, trimmed_source_length = state["source_to_target"].size()
            
            #input length: state["source_token_ids"][0, :].size()
        ...
        ### Implementation of only copying from input but with combining copy log probs from input ###
            #Will lessen the effect of combining as fewer duplicate instances to combine; is more obsolete and rather for testing
            #replaced input_len in for loop
            #replaced trimmed_source_length with input_len in first if statement
    
        elif not self._do_not_combine_copy_log_probs and self._do_not_copy_out_of_extractions: 

            _ , input_len = state["source_token_ids"].size()
            source_token_ids = state["source_token_ids"]
        ...
        ### Implementation with only copying out of the input instead of all extractions without combining copy log probs ###
        #removed if statements; replaced input_len in for loop

        elif self._do_not_combine_copy_log_probs and self._do_not_copy_out_of_extractions:
