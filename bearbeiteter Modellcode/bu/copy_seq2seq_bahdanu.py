...
from torch.nn.modules.rnn import LSTMCell, GRUCell, GRU
...
class CopyNetSeq2Seq(Model):
    ...
    def __init__(self,
                 ...
                 reduced_copy_attention : bool = False,  #for enabling/disabling param block below
                 do_combine_copy_log_probs: bool = False,
                 add_copy_to_gen_log_probs: bool = False,

                 reduced_extr_copying:bool = False, #for enabling/disabling param block below
                 copy_out_of_n_last_extr: int = -1,  #-1: regular copying out of all extractions #0: only input sentence
                 #forced_copy_from_input_sent: bool = True,
                 selective_read_out_of_n_last_extr: int = -1, #-1: regular selective read out of all extractions #0: only input sentence 
                                                              #TODO This may require a well documented Train approach for me to understand
                                                              #TODO do it; it will need another training approach adjusted to reduced extr_copying
                                                              # TODO may also test it with regular training
                 #forced_sel_read_from_input_sent: bool = True    
                 ) -> None:

        super().__init__(vocab)
        
        self._reduced_copy_attention = reduced_copy_attention
        self._add_copy_to_gen_log_probs = add_copy_to_gen_log_probs
        self._do_combine_copy_log_probs = do_combine_copy_log_probs

        self._reduced_extr_copying = reduced_extr_copying
        self._copy_out_of_n_last_extr = copy_out_of_n_last_extr
        #self._forced_copy_from_input_sent = forced_copy_from_input_sent
        self._selective_read_out_of_n_last_extr = selective_read_out_of_n_last_extr
        #self._forced_sel_read_from_input_sent = forced_sel_read_from_input_sent

        #the following configuration errors prevent the wrong use of parameters and spare some coded conditions 


        if self._selective_read_out_of_n_last_extr != -1:
            raise NotImplementedError('the parameter _selective_read_out_of_n_last_extr is not implemented yet.')

        if self._copy_out_of_n_last_extr < -1:
            raise ConfigurationError("copy_out_of_n_last_extr can either be -1 (disabled) or greater than -1")

        if not reduced_copy_attention and (do_combine_copy_log_probs or 
                                           add_copy_to_gen_log_probs):
            raise ConfigurationError("Boolean parameters for reduced_copy_attention are set to True while reduced_copy_attention is not enabled")

        if add_copy_to_gen_log_probs and do_combine_copy_log_probs:
            raise ConfigurationError("Params add_copy_to_gen_log_probs and do_combine_copy_log_probs cannot be set to True at the same time. This is equal to the normal")


        # elif (reduced_extr_copying == False) and (copy_out_of_n_last_extr > -1 or
        #                                          selective_read_out_of_n_last_extr > -1):
        #     raise ConfigurationError("Boolean parameters for reduced_extr_copying are changed, however reduced_extr_copying is not enabled")

        # elif not (selective_read_out_of_n_last_extr > -1) and (forced_copy_from_input_sent == False):
        #      raise ConfigurationError("forced_copy_from_input_sent should not be set to False when forced_copy_from_input_sent > -1. \n" +
        #                               "Copying from the input sentence should always be considered when doing the default copying from all extractions.")

        # elif not (copy_out_of_n_last_extr > -1) and (forced_sel_read_from_input_sent == False):
        #      raise ConfigurationError("forced_sel_read_from_input_sent should not be set to False when selective_read_out_of_n_last_extr > -1. \n" +
        #                               "Applying the selective read to the input should always be considered for the regular selective read.")

        # elif (copy_out_of_n_last_extr == 0 and forced_copy_from_input_sent == False):
        #     raise ConfigurationError("copy_out_of_n_last_extr is set to 0 to copy only from the input but forced_copy_from_input_sent is set to False")
        # elif (selective_read_out_of_n_last_extr == 0 and forced_sel_read_from_input_sent == False):
        #     raise ConfigurationError("selective_read_out_of_n_last_extr is set to 0 to read only from the input but forced_sel_read_from_input_sent is set to False")
        ...
        global START_SYMBOL,END_SYMBOL # Needed for lstm_append - could be done for all - but issue is lstm_single is trained with old START, END symbols
        if self._append: 
        ...
        elif self._decoder_type == 'gru':
            self._decoder_cell = GRU(self.decoder_input_dim, self.decoder_output_dim, num_layers=self._decoder_layers, batch_first=True)
    ...
    def forward_single(self, source_tokens, source_token_ids, source_to_target, metadata, target_tokens, target_token_ids, optimizer):
        #print('forward_single_encode___')
        state = self._encode(source_tokens)
        state["source_token_ids"] = source_token_ids
        state["source_to_target"] = source_to_target
        if self._coverage:
    ...
    def test_append(self, source_tokens, source_token_ids, source_to_target):
        ...
        stop = False   
        global num_extractions #global for knowing the num of extractions for _copy_out_of_n_last_extr and _selective_read_out_of_n_last_extr
        num_extractions = 0
        completed = np.array([0]*batch_size)
        other_predictions = []
        other_log_probs = []
        global max_extr_lenghts #global for knowing how much tokens to consider for _copy_out_of_n_last_extr and _selective_read_out_of_n_last_extr
        max_extr_lenghts = [] #max length is 10; values from 0 to 50 (very often 50 as it is the max observed length of an extraction)
                              # is adjusted in function gather_final_log_probs
    ...
    def _encode(self, source_tokens: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        #a = torch.randn(128, 500, 256).to('cuda')
        #t1 = time.time()
        #a = a.to('cpu')
        #print(f'test: .to(cpu): {time.time() - t1}')
        try:
            source_tokens_cpu = source_tokens['tokens'].cpu()
    ...
    def _decoder_step(self,
        ...
        if self._decoder_type == 'lstm' or 'gru':
            if self._coverage : #not tested with GRU decoder
            ...
            if self._decoder_type == 'lstm':
                ...
                _, state["decoder_hidden_all"] = self._decoder_cell(projected_decoder_input.unsqueeze(1), state["decoder_hidden_all"]) 

                state["decoder_hidden"] = state["decoder_hidden_all"][-1]
                # (decoder_layers, batch_size, output_dim) --> (batch_size, decoder_layers * output_dim)
                state["decoder_hidden_all"] = state["decoder_hidden_all"].transpose(0,1).contiguous().view(-1, self._decoder_layers*self.decoder_output_dim)
    ...
    def _get_input_and_selective_weights(self,
        ...
        # shape of mask: (group_size, trimmed_source_length)
        #print(state["source_token_ids"])
        #if self._selective_read_out_of_n_last_extr > -1: 
          ##  mask = (source_only_copied | source_copied_and_generated) & True #TODO
           # if (not self._forced_sel_read_from_input_sent):
                #pass
        #else :
    ...
    def _gather_final_log_probs(self,
        ...
        #max_extr_lenghts.append(state["source_to_target"].size()[1])    #global initialized in self.test_append()
        #print(f'max_extr_lenghts: {max_extr_lenghts} WARNING: needs to be 500? is there [:-1]?')
        #print(f'compare current stt to length: {state["source_to_target"][0, :]}, {state["source_to_target"][0, :].size()}')
        #print(f'compare current st_ids to length: {state["source_token_ids"][0, :]}, {state["source_token_ids"][0, :].size()}')

        t1 = time.time()
        
        ### normal gather final log probs ###

        if (not self._reduced_copy_attention) and (not self._reduced_extr_copying):          
        ...
        ### reduced copy attention only ###

        elif self._reduced_copy_attention and (not self._reduced_extr_copying):
            #no copy_log_probs get added to the generative_log_probs so that they wont get changed
            #those copy_log_probs that would get added to the generative_log_probs are masked out like above

            #the copy_log_probs are not combined which keeps multiple occurences of specific copy_log_probs uncollected and not zeroed out
            #this would may disrupt the copymode and decrease performance in exchange for a computational speed-up

            #This function code is extremely slow and hardly parallelizable/optimizable 
            #so this method is tried in order to reduce the computation time by cutting out the slow elements of the copy-attention implementation

            if self._add_copy_to_gen_log_probs:
                #To test the effect of adding the copy logprobs to the generative logprobs and zeroing them out
                _, trimmed_source_length = state["source_to_target"].size()
                source_token_ids = state["source_token_ids"]
                modified_log_probs_list: List[torch.Tensor] = []
                for i in range(trimmed_source_length):
            ...
            elif self._do_combine_copy_log_probs:
                #I run the second part of the code to test if not combining the copy_log_probs significantly harms performance.
                _, trimmed_source_length = state["source_to_target"].size()
                source_token_ids = state["source_token_ids"]
                modified_log_probs_list: List[torch.Tensor] = [] 
                for i in range(trimmed_source_length):
            ...
            ### reduced extraction copying ###

            elif not self._reduced_copy_attention and self._copy_out_of_n_last_extr > -1: 
                #Note:      
                #      The copy attention takes a big role for generating by copying non-oov tokens. 
                #      (observation: when not adding the non-oov clps to the generative log probs it kills performence)
                #      (observation: not combining the oov copy logprobs however changes almost nothing for AUC and F1 and reduces test time by half
                #      [but there is a slight Prec/Rec trade and possibly a minorly disturbed selective read])
                #      So an idea to eliminate the long runtime issue could be to copy out of less extractions or even only from the input   
                #      May the copy attention will then copy more directly more important vocab than mainly augmenting the generative task.
                #      In my understanding it should also not be mandatory to copy the already generated token again 
                #      (long test time; mainly source vocab should only be copyied for OIE and not other generated vocab)
                #      TODO create a different training objective for only copying out of source and 
                #           also for copying some recent extractions alongside the input. 
                #      TODO test it 


                # num_extractions is a global defined in self.test_append
                # below the extractions on which to apply gather_final log probs are always the last available extractions <= self._copy_out_of_n_last_extr
                considered_extr = self._copy_out_of_n_last_extr if (self._copy_out_of_n_last_extr >= num_extractions) else num_extractions
                if considered_extr > 0: #then gather_final_log_probs is only apllied fully to the last number of considered_extr and the input source
                        # do do consider gflp for n extr
                    pass
                    
                else:  #then gather_final_log_probs is only apllied fully to the copy log probs from the input source
                    group_size, trimmed_source_length = state["source_to_target"].size()
                    source_token_ids = state["source_token_ids"] 
                    modified_log_probs_list: List[torch.Tensor] = []            
                    # if num_extractions > 0 then the last token of precious extractions is deleted in test_append
                    # at line: 'new_unp_append_tokens.append(unp_append_tokens[bs][:-1] + unp_prediction_tokens_bs.tolist())'
                    # max_extr_lengths[0] is the source length of the input sent



                    source_input_len = max_extr_lenghts[0] - 1*(num_extractions != 0)
                    cons_extr_length = #TODO
                    input_mask = torch.ones([group_size, source_input_len])
                    not_considered_mask = torch.zeros([group_size, ])
                    considered_extr_mask = torch.ones(group_size, )
                    considered_mask =             # source_input_len[0]#None #TODO 




                    for i in range(source_input_len):
                        copy_log_probs_slice = copy_log_probs[:, i]
                        source_to_target_slice = state["source_to_target"][:, i]
                        copy_log_probs_to_add_mask = (source_to_target_slice != self._oov_index).float()
                        copy_log_probs_to_add = copy_log_probs_slice + (copy_log_probs_to_add_mask + 1e-45).log()
                        ...
                        if i < (trimmed_source_length - 1):
                            ...
                            # the combining with not considered extractions is disabled
                            # still will be taking a lot of time to compute
                            # TODO test: with reduced copy attention: no combining at all!
                            source_future_occurences = ((source_token_ids[:, (i+1):] == source_token_ids[:, i].unsqueeze(-1)) * considered_mask).float() 
                            future_copy_log_probs = copy_log_probs[:, (i+1):] + (source_future_occurences + 1e-45).log()
                    ...
                    #not considered copy log probs for none oov-tokens are masked anyway so the selective read is not disrupted.
                    #shape: (group_size, extraction_source_length) TODO
                    not_considered_clps = copy_log_probs[:, inp_extr_len:]
                    not_considered_stt = state["source_to_target"][:, inp_extr_len:]
                    clps_to_add_mask = (not_considered_stt != self._oov_index).float()
                    left_over_clps = not_considered_clps + (1.0 - clps_to_add_mask + 1e-45).log()
                    #incase of no further extractons it wont append anything but empty lists
                    modified_log_probs_list.append(left_over_clps)
                    modified_log_probs_list.insert(0, generation_log_probs)
                    modified_log_probs = torch.cat(modified_log_probs_list, dim=-1)

            ### reduced copy attention + reduced extraction copying ###

            elif self._reduced_copy_attention and self._reduced_extr_copying:
                raise NotImplementedError('A reduced copy attention is not implemented alongside a reduced extraction copying')

            else:   
                #fully reduced copy attention like described above
                # observation: kills performence; reduced test time from 500 seconds (normal model)
                # to 50 seconds (which is the time from self.source_embedder() in encode_)
                copy_log_probs_to_add_mask = (state["source_to_target"] != self._oov_index).float()
                copy_log_probs = copy_log_probs + (1.0 - copy_log_probs_to_add_mask + 1e-45).log()
                modified_log_probs = torch.cat([generation_log_probs, copy_log_probs], dim=-1)

            t2 = time.time()
            #print(f'g_f_logprobs : {t2-t1} s')
